...
# Apply Encoder/Decoder Convolutions Blocks
filters_reduction = 128  

# Add 1x1-sized kernel convolution at the beginning
conv_layer = Conv2D(filters=filters_reduction, kernel_size=(1, 1))(input_layer)
...
for _ in range(3):
    ...
    for _ in range((9-1)//2):
        conv_layer = Conv2D(filters=filters_reduction, kernel_size=(3, 3), padding="same")(conv_layer)  
        conv_layer = BatchNormalization()(conv_layer)
        conv_layer = Activation("tanh")(conv_layer)
        conv_layer = Dropout(0.2)(conv_layer)
    
    # Change filters values to 𝑛 in the convolutions that will be compressed
    for _ in range((9-1)//2):
        conv_layer = Conv2D(filters=filters_reduction, kernel_size=(3, 1), padding="same")(conv_layer)  
        conv_layer = Conv2D(filters=filters_reduction, kernel_size=(1, 3), padding="same")(conv_layer)  
        conv_layer = BatchNormalization()(conv_layer) 
        conv_layer = Activation("tanh")(conv_layer) 
        conv_layer = Dropout(0.2)(conv_layer)

    # Add 1x1-sized kernel convolution at the end
    conv_layer = Conv2D(filters=128, kernel_size=(1, 1))(conv_layer)  
    conv_layer = BatchNormalization()(conv_layer)  
    conv_layer = Activation("relu")(conv_layer)  
    conv_layer = Dropout(0.2)(conv_layer)  
